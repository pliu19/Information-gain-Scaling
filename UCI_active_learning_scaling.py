...
def load_data():
    ...
    X = np.loadtxt(file_path, dtype=float, delimiter=",", skiprows=1, usecols=read_cols)
    y = np.loadtxt(file_path, dtype=int, delimiter=",", skiprows=1, usecols=(class_index,), converters={class_index: lambda x: classes.index(x)})
...
def main():
    ...
    parser.add_argument('-filename', default="ionosphere_IG", help='The name of file.')
    ...
    for i in args.picking_arguments:
        ...
        print "     Scaling method: %s" % args.feature_scaling
